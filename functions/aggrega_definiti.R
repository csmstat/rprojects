aggrega_definiti <- function(dt, keyby_cols) {
  
  tmp <- dt[, lapply(.SD, function(x) sum(as.numeric(x), na.rm = T)), keyby = keyby_cols, .SDcols = c("Definiti")]
  tmp <- melt(tmp, id.vars = keyby_cols, measure.vars = "Definiti")
